...
const TEST_TYPES = [
  {
    id: "overall-health-screening",
    name: "Overall Health Screening",
  ...
  {
    id: "chronic-condition-monitoring",
...
const STEP_LABELS = ["Test Type", "Date", "Time", "Details", "Review"];
...
function BookTestContent() {
  ...
  const [testType, setTestType] = useState<typeof TEST_TYPES[number] | null>(null);
  ...
  const fetchAvailability = async (d: { year: number; month: number; day: number }) => {
    ...
    try {
      ...
      const res = await fetch(`/api/test-bookings/availability?date=${dateStr}`);
  ...
  const canProceedStep1 = !!testType;
  ...
  const handleSubmit = async () => {
    if (!testType || !date || !timeSlot || !name.trim() || !phone.trim()) return;
    const digits = phone.replace(/\D/g, "");
                  ...
                  <div key={idx} className="flex items-center flex-1">
                    ...
                    {i < 4 && (
                      <div className="flex-1 h-0.5 mx-1 min-w-[8px] bg-border rounded">
                        <div
                          className={`h-full rounded transition-all ${completed ? "bg-teal" : "bg-border"}`}
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div
              ...
              className="space-y-6"
            >
              <h2 className="text-lg font-semibold text-charcoal">Select Test Type</h2>
              <div className="grid gap-4 sm:grid-cols-2">
                {TEST_TYPES.map((test) => (
                  <button
                    ...
                    className={`p-5 rounded-xl border-2 text-left transition group ${
                      testType?.id === test.id
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      <div className={`inline-flex items-center justify-center w-10 h-10 rounded-lg transition-colors ${
                        testType?.id === test.id ? "bg-teal text-white" : "bg-cream text-orange group-hover:bg-teal/10 group-hover:text-teal"
                      }`}>
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={test.icon} />
                        </svg>
                      </div>
                      <span className={`font-semibold ${testType?.id === test.id ? "text-teal" : "text-charcoal"}`}>
                        {test.name}
                      </span>
                    </div>
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="px-6 py-2.5 rounded-lg bg-teal text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-teal/90 transition"
                >
                  Next
                </button>
              </div>
                        ...
                        disabled
                          ...
                          : selected
                            ...
                            : today
                              ...
                              : "border-border bg-card hover:bg-cream hover:border-teal/40 text-charcoal font-semibold"
                      }`}
                    >
                      {d}
                    </button>
            ...
            >
              ...
              {availabilityLoading ? (
                <div className="flex items-center justify-center py-12">
                  <svg className="animate-spin w-6 h-6 text-teal mr-3" fill="none" viewBox="0 0 24 24">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg border px-4 py-3 focus:ring-2 focus:ring-teal focus:border-teal outline-none transition ${
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Age"
                    className="w-full rounded-lg border border-border px-4 py-3 focus:ring-2 focus:ring-teal focus:border-teal outline-none transition"
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Any specific tests or conditions to mention"
                    rows={3}
                    className="w-full rounded-lg border border-border px-4 py-3 focus:ring-2 focus:ring-teal focus:border-teal outline-none transition resize-none"
                  />
                </div>
              </div>
              <div className="flex justify-between pt-4">
          ...
          {step === 5 && (
            ...
            >
              ...
              <div className="rounded-xl border border-border bg-card p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted uppercase">Test Type</p>
                    <p className="font-medium text-charcoal">{testType?.name}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => goToStep(1)}
                ...
                {notes && (
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-muted uppercase">Additional Notes</p>
              ...
              <div className="flex justify-between pt-4">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-lg bg-teal text-white font-medium disabled:opacity-50 hover:bg-teal/90 transition flex items-center gap-2"
                >
                  {submitLoading ? (
                    <>
          ...
          {step === 6 && (
            ...
            >
              ...
              <h2 className="text-2xl font-semibold text-charcoal mb-2">Test Booking Submitted!</h2>
              <p className="text-muted mb-4">Your booking reference</p>
              <p className="text-2xl font-bold text-teal mb-2">{bookingId}</p>
              {assignedTime && (
              ...
              <p className="text-body max-w-md mx-auto mb-8">
                Your diagnostic test booking has been submitted! The clinic will contact you to confirm your appointment.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button
                  type="button"
                  onClick={resetForm}
                  className="px-6 py-3 rounded-lg bg-teal text-white font-medium hover:bg-teal/90 transition"
                >
                  Book Another Test
                </button>
                <Link
